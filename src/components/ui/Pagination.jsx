...
const Pagination = ({ currentPage, totalPages, onPageChange, itemsPerPage, setItemsPerPage }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6">
      ...
      <div className="flex items-center gap-2">
        <Button onClick={handlePrevious} disabled={currentPage === 1} variant="secondary" className="text-sm">Previous</Button>
        <div className="flex gap-1">
          {startPage > 1 && <>
            <Button onClick={() => onPageChange(1)} variant="secondary" className="text-sm">1</Button>
          ...
          {endPage < totalPages && <>
            {endPage < totalPages - 1 && <span className="self-center">...</span>}
            <Button onClick={() => onPageChange(totalPages)} variant="secondary" className="text-sm">{totalPages}</Button>
          </>}
        </div>
        <Button onClick={handleNext} disabled={currentPage === totalPages} variant="secondary" className="text-sm">Next</Button>
      </div>
    </div>
  );
};
